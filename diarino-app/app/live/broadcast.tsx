...
function BroadcastFlow() {
  ...
  // ↔ اختيار الكاميرا الأمامية/الخلفية قبل بدء البث — القيمة دي بتتبعت
  // فعليًا لـ LiveKitRoom (video={{ facingMode }}) عشان الكاميرا تفتح
  // على الاختيار الصحيح من أول لحظة، بدل ما تفتح دايمًا أمامية ثم
  // تحتاج تدوير يدوي بعد الاتصال.
...
function BroadcasterLiveView({
  ...
  // ↔ نفس نمط الفلترة المستخدم فى app/live/[id].tsx (remoteTrackRef) بالظبط،
  // لكن هنا بنلاقي تراك المذيع المحلي (isLocal=true) بدل تراك المشاهد
  // البعيد. ده بيدّينا type ضيق وصحيح (`publication.track: Track | undefined`
  // الحقيقي من livekit-client) بدل type يدوي تقريبي، وهو اللي محتاجه
  // switchCamera() تحت فى flipCamera().
