...
export default function EditProfileScreen() {
  ...
  async function handleSave() {
    ...
    // ↔ تحقق من صيغة تاريخ الميلاد قبل الإرسال — عمود birth_date فى
    // قاعدة البيانات من نوع date، فأي نص مش تاريخ صحيح (أو فاضي جزئيًا)
    // كان بيوصل لـ Postgres ويرجّع خطأ عام يوقف حفظ كل الحقول التانية
    // معاه من غير ما المستخدم يفهم السبب. بنتحقق هنا بدل ما نسيب الخطأ
    // يوصل من قاعدة البيانات.
    const trimmedBirthDate = birthDate.trim();
    if (trimmedBirthDate) {
      const isValidFormat = /^\d{4}-\d{2}-\d{2}$/.test(trimmedBirthDate);
      const parsedDate = isValidFormat ? new Date(trimmedBirthDate) : null;
      const isRealDate = !!parsedDate && !Number.isNaN(parsedDate.getTime()) && trimmedBirthDate === parsedDate.toISOString().slice(0, 10);
      if (!isRealDate || parsedDate!.getTime() > Date.now()) {
        Alert.alert(t("تاريخ الميلاد غير صحيح"), t("من فضلك أدخل تاريخ الميلاد بصيغة YYYY-MM-DD."));
        return;
      }
    }
    setSaving(true);
