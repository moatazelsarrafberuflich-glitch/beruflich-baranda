...
export default function EditProfileScreen() {
  ...
  // ↔ تاريخ الميلاد بقى ٣ قوائم منفصلة (يوم/شهر/سنة) بدل حقل نصي حر —
  // كان بيسبب أخطاء صيغة صامتة توقف الحفظ بالكامل. القوائم دي بتضمن
  // قيمة صحيحة دايمًا أو مفيش قيمة خالص، مفيش احتمال لصيغة غلط.
  ...
  //
  // ↔ إصلاح: "الحفظ لا يعمل" — كان الكود بيحط profile.phoneE164 (الرقم
  // الكامل زي +201012345678) مباشرة جوه localNumber من غير تقسيم. أي
  // مستخدم عنده رقم هاتف محفوظ مسبقًا ويحاول يعدّل أي حقل تاني (حتى
  // لو ملموسش رقم الهاتف خالص) كان handleSave بيحاول يتحقق من الرقم
  // ده كأنه رقم محلي، يفشل التحقق (رقم أطول من المتوقع لأنه فيه كود
  // الدولة اتكرر جواه)، ويوقف عملية الحفظ بالكامل قبل ما توصل أصلاً
  // لـ update.mutateAsync — يعني كل الحقول التانية (اسم المستخدم، الاسم،
  // تاريخ الميلاد...) بتتفقد معاه. splitE164 هنا بيرجّع الرقم لصورته
  // المحلية الصحيحة (زي ما PhoneInput متوقعها بالظبط) وقت التحميل.
  useEffect(() => {
  ...
  // ↔ بيانات القوائم الثلاث لتاريخ الميلاد. عدد أيام الشهر بيتحسب فعليًا
  // حسب الشهر/السنة المختارين (بيراعي السنة الكبيسة) عشان يبقى مستحيل
  // تختار تاريخ غير موجود أصلاً (زي ٣٠ فبراير) بدل ما نتحقق منه بعدين.
  ...
  const yearOptions = useMemo(
    () => Array.from({ length: 100 }, (_, i) => String(currentYear - 13 - i)), // من عمر ١٣ سنة كحد أدنى
    [currentYear]
  );
  ...
  // ↔ اسم المستخدم بقى بيتكوّن تلقائيًا من الاسم الأول + الثاني (مطلوب
  // إزالة حقل إدخاله يدويًا)، بحروف إنجليزية صغيرة وبدون مسافات/رموز،
  // مع إضافة رقم فى الآخر لو الاسم ده مأخوذ فعليًا (فريد بحسب فهرس
  // profiles_username_unique_idx). suffix بيتحدد من آخر ٤ خانات من
  // معرّف المستخدم نفسه — قيمة ثابتة لكل مستخدم (مش عشوائية فتتغيّر كل
  // مرة)، فمفيش داعي لأي محاولات إعادة إرسال متكررة.
  function deriveUsername(first: string, last: string, userId: string): string | undefined {
    const base = `${first}${last}`
      .toLowerCase()
      .normalize("NFD").replace(/[\u0300-\u036f]/g, "") // إزالة الحروف المُشكَّلة/إكسنتات
      .replace(/[^a-z0-9\u0600-\u06FF]/g, "");
  ...
  async function handleSave() {
    ...
    // ↔ تاريخ الميلاد بقى مبني من ٣ قوائم (يوم/شهر/سنة) مش نص حر، فمستحيل
    // يوصل بصيغة غلط لقاعدة البيانات — الشيء الوحيد المطلوب التحقق منه
    // هو إن المستخدم اختار الثلاثة مع بعض أو مسيبهم فاضيين تمامًا، مش
    // جزء منهم بس.
    ...
    try {
      ...
      try {
        ...
        // ↔ اسم المستخدم بقى مُشتق تلقائيًا، مش بيدخله المستخدم بنفسه —
        // لو حصل تعارض نادر (احتمال ضئيل جدًا) بنعيد المحاولة تلقائيًا
        // برقم إضافي فى الآخر من غير ما نزعج المستخدم برسالة عن حقل هو
        // أصلاً مش شايفه فى الشاشة.
